...
import { useUpdateMovie } from '../../model/useUpdateMovie'
...
export const UpdateMovieForm = (props: UpdateMovieFormProps) => {
	const { movieId, defaultValues, children, onCloseDialog } = props
	...
	const { updateMovie, error, isPending } = useUpdateMovie(onCloseDialog)
	...
	const handleUpdateMovie = (formData: FormSchema) => {
	...
	return (
		<FormProvider {...methods}>
			<form
				onSubmit={methods.handleSubmit(handleUpdateMovie)}
			...
			>
				{error && (
					...
					>
						{getErrorMessage(error, 'Error while updating movie')}
					</Alert>
				)}
